...
class Jevix{
      ...
      foreach($tags as $tag){
        if(!isset($this->tagsRules[$tag])) {
          if($createIfNoExists){
            ...
            throw new DdspException("Тег $tag отсутствует в списке разрешённых тегов");
          }
        }
    ...
    public function cfgAllowTagParams($tag, $params){
      if(!isset($this->tagsRules[$tag])) throw new DdspException("Тег $tag отсутствует в списке разрешённых тегов");
    ...
    public function cfgSetTagChilds($tag, $childs, $isContainerOnly = false, $isChildOnly = false){
      ...
      foreach($childs as $child){
        ...
        if(!isset($this->tagsRules[$child])) throw new DdspException("Тег $child отсутствует в списке разрешённых тегов");
  ...
  function cfgSetTagParamsAutoAdd($tag, $params){
    if(!isset($this->tagsRules[$tag])) throw new DdspException("Tag $tag is missing in allowed tags list");
    if(!is_array($params)) $params = array($params);
    ...
    protected function restoreState($index = null){
      if(!count($this->states)) throw new DdspException('Конец стека');
      if($index == null){
        $state = array_pop($this->states);
      } else {
        if(!isset($this->states[$index])) throw new DdspException('Неверный индекс стека');
          ...
          if(is_string($paramAllowedValues)){
            switch($paramAllowedValues){
                ...
                case '#link':
                  ...
                  /* && !preg_match('/^(mailto:|\#)/ui', $value) ------------ якорь */
                  ) $value = 'http://'.$value;
                  break;
